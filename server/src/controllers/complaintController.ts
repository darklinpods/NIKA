...
export const extractComplaintElements = async (req: Request, res: Response) => {
  ...
  "requestsAndFacts": "完整综合的【诉讼请求】和【事实与理由】段落合并。保持原有的清晰条理和分段。"
        ...
        // Clean up markdown brackets if any
        dataStr = dataStr.replace(/```json/gi, '').replace(/```/gi, '').trim();
        
        try {
            const parsed = JSON.parse(dataStr);
            res.json({ success: true, data: parsed });
        } catch (e) {
            console.error("Failed to parse gemini JSON", dataStr);
...
export const generateComplaintDocx = async (req: Request, res: Response) => {
    try {
        ...
        if (!templatePath) {
             ...
             return res.status(500).json({ error: `模板文件未找到: ${templateFileName}` });
        }

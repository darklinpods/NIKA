                        ...
                        <div className="flex flex-col gap-6 max-w-4xl">
                            ...
                            <div className={`inline-flex p-1.5 rounded-xl self-start ${isDark ? 'bg-slate-950' : 'bg-slate-100'}`}>
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${sourceType === 'text'
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${sourceType === 'case'
                                            ? (isDark ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-indigo-700 shadow-md')
                                            : (isDark ? 'text-slate-400 hover:text-slate-200' : 'text-slate-500 hover:text-slate-700')
                                        }`}
                                >
                            ...
                            {sourceType === 'text' ? (
                                <textarea
                                    ...
                                    className={`w-full h-56 px-5 py-4 rounded-xl border-2 outline-none font-sans text-sm resize-none custom-scrollbar transition-all ${isDark ? 'bg-slate-900 border-slate-800 text-slate-200 focus:border-indigo-500/50' : 'bg-white border-slate-200 focus:border-indigo-400'
                                        }`}
                                />
                            ) : (
                                <div className={`w-full h-56 rounded-xl border-2 border-dashed flex flex-col items-center justify-center p-8 transition-colors ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-300'
                                    }`}>
                                    ...
                                    {selectedCaseId && (() => {
                                        const c = casesList.find(x => x.id === selectedCaseId);
                                        return c?.factSheetUpdatedAt ? (
                                            <p className={`text-xs mt-1.5 px-2 py-1 rounded-lg ${isDark ? 'bg-amber-900/30 text-amber-300' : 'bg-amber-50 text-amber-700'}`}>
                                                ⚠️ 本次生成将基于 <strong>{new Date(c.factSheetUpdatedAt).toLocaleString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}</strong> 的案件详情
                                            </p>
                                        ) : null;
                                    })()}
                                </div>

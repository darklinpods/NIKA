...
* **📑 案件详情与文书管理**：记录案件各项基本信息（客户、法院等），支持无限层级的子任务管理以及多端 Markdown 文档归档。
...
npm run dev:client
```
随后在浏览器中访问 `http://localhost:5173` 即可开始使用。
...
├── client/                 # 前端 React 源码
│   ├── src/
│   │   ├── components/     # UI 视图组件 (BoardView, TaskCard 等)
│   │   ├── hooks/          # 核心业务逻辑 (useTaskOperations, useAIFeatures)
│   │   ├── providers/      # 全局上下文 (AppProvider, ConfirmProvider)
│   │   └── services/       # API 接口统一请求封装 (api.ts)
├── server/                 # 后端 Node.js 源码
│   ├── src/
│   │   ├── controllers/    # 路由控制器，处理 HTTP 响应规范
│   │   ├── services/       # 抽象的 Prisma 数据库查询层 (caseService.ts)
│   │   ├── routes/         # Express API 路由定义
│   │   └── prisma/         # 数据库 Schema 结构文件
```

## 🤝 开发指南
本项目近期经历了一次大规模的解耦重构：
1. **客户端**将庞大的原始 Hook 拆分为四个领域专用的 Hooks（状态管理、增删改查、拖拽流转、AI 交互）。
2. **服务端**完善了 Controller 与 Service 的分层结构。

全部核心代码均已补充完善的**简体中文注释**，大幅降低了二次开发门槛。如需增加新功能，可直接跳转阅读 `client/hooks` 与 `server/src/services` 下的文件。
